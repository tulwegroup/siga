...
<<<<<<< HEAD
    ...
    const transformedEntity = {
      ...
      entityId: entity.id,
      name: entity.name,
      category: entity.type || entity.category || 'SOE',
      sector: entity.sector,
      parentMinistry: entity.sector.includes('Finance') ? 'Ministry of Finance' : 
                     entity.sector.includes('Energy') ? 'Ministry of Energy' :
                     entity.sector.includes('Transport') ? 'Ministry of Transport' :
                     entity.sector.includes('Agriculture') ? 'Ministry of Agriculture' :
                     entity.sector.includes('Health') ? 'Ministry of Health' :
                     entity.sector.includes('Communications') ? 'Ministry of Communications' :
                     entity.sector.includes('Trade') ? 'Ministry of Trade and Industry' :
                     entity.sector.includes('Tourism') ? 'Ministry of Tourism' :
                     entity.sector.includes('Revenue') ? 'Ministry of Finance' :
                     entity.sector.includes('Regulation') ? 'Office of the President' :
                     entity.sector.includes('Governance') ? 'Office of the President' :
                     entity.sector.includes('Education') ? 'Ministry of Education' :
                     entity.sector.includes('Defense') ? 'Ministry of Defense' :
                     entity.sector.includes('Foreign Affairs') ? 'Ministry of Foreign Affairs' :
                     'Office of the President',
      status: entity.status,
      contactEmail: entity.contactEmail || `info@${entity.id.toLowerCase()}.gov.gh`,
      contactPhone: entity.contactPhone || '+233 30 000 0000',
      website: entity.website ? `https://${entity.website}` : `https://${entity.id.toLowerCase()}.gov.gh`,
      description: entity.description,
      establishedDate: entity.establishedYear ? `${entity.establishedYear}-01-01` : '2000-01-01',
      boardMembers: entity.boardMembers?.map(bm => ({
        id: bm.id,
        name: bm.name,
        position: bm.position,
        isChairperson: bm.position.toLowerCase().includes('chairman'),
        appointmentDate: bm.appointedDate || '2020-01-15',
        termEndDate: bm.termEndDate || '2024-12-31',
        email: bm.email ? bm.email : `${bm.name.toLowerCase().replace(' ', '.')}@${entity.id.toLowerCase()}.gov.gh`
      })) || [],
      riskScores: entity.riskScores?.map(rs => ({
        period: rs.period,
        overallScore: rs.overallScore,
        financialRisk: rs.financialRisk,
        operationalRisk: rs.operationalRisk,
        governanceRisk: rs.governanceRisk,
        complianceRisk: rs.complianceRisk
      })) || [],
      complianceLogs: entity.complianceLogs?.map(cl => ({
        id: cl.id,
        requirement: cl.requirement,
        category: cl.category,
        status: cl.status,
        dueDate: cl.dueDate,
        completedDate: cl.completedDate,
        assignedTo: cl.assignedTo,
        notes: cl.notes
      })) || [],
      kpiData: entity.kpiData?.map(kpi => ({
        period: kpi.period,
        revenue: kpi.revenue,
        profit: kpi.profit,
        assets: kpi.assets,
        ...
        roa: kpi.roa,
        roe: kpi.roe,
        debtToEquity: kpi.debtToEquity,
        employeeCount: kpi.employeeCount,
        serviceDeliveryIndex: kpi.serviceDeliveryIndex,
        customerSatisfaction: kpi.customerSatisfaction
      })) || [],
      dividends: entity.dividends?.map(d => ({
        year: d.year,
        amountDeclared: d.amountDeclared,
        amountPaid: d.amountPaid,
        status: d.status
      })) || [],
      guarantees: entity.guarantees?.map(g => ({
